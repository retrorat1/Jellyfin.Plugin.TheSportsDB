...
def main() -> int:
    ...
    print(
        f"Rate limit:        {args.max_rpm} requests / 60s "
        f"(~{limiter.min_interval_seconds:.2f}s between calls)"
    )
    ...
    for index, (team_id, name, short_name, alternative_names) in enumerate(
        rows, start=1
    ):
        try:
        ...
        new_alternate = merge_alternates(
            name, short_name, alternative_names, api_alternate
        )
        ...
        if memberships:
            ...
            primary_league = next(
                lid for lid, is_primary in memberships if is_primary
            )
            conn.execute(
